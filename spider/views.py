...
def getResultByName(name):
    ...
    res.encoding = 'utf-8'
    obj = re.compile(r'"><a href="(?P<href>.*?)".*?">(?P<title>.*?)</a>.*?<span class="s4">(?P<author>.*?)</span>',
                     re.S)
...
def get_more(request):
    ...
    obj = re.compile(
        '"item masonry_brick">.*?href="(?P<href>.*?)".*?data-original="(?P<src>.*?)".*?alt="(?P<name>.*?)"', re.S)
...
def getImageByname(name):
    ...
    obj = re.compile(
        'img_cont hoff.*?src="(?P<small>.*?)".*?图像结果.*?href="(?P<bighref>.*?)" h=.*?aria-label="(?P<name>.*?)"', re.S)
    results = []
    smalls = []
    bighrefs = []
    names = []
...
def getNextByname(name, index):
    ...
    res = requests.get(url, headers=headers, params=para)
    obj = re.compile(
        'img_cont hoff.*?src="(?P<small>.*?)".*?图像结果.*?aria-label="(?P<name>.*?)".*?href="(?P<bighref>.*?)" h=', re.S)
    results = []
    smalls = []
    bighrefs = []
    names = []
...
def get_wallpaper(request):
    ...
    if request.GET.get("update"):
        urls = ['https://mm.enterdesk.com/dalumeinv/', 'https://mm.enterdesk.com/rihanmeinv/',
                'https://mm.enterdesk.com/gangtaimeinv/',
                'https://mm.enterdesk.com/dongmanmeinv/', 'https://mm.enterdesk.com/qingchunmeinv/',
                'https://mm.enterdesk.com/keaimeinv/',
                'https://www.enterdesk.com/zhuomianbizhi/fengjing/',
                'https://www.enterdesk.com/zhuomianbizhi/dongmankatong/',
                'https://sj.enterdesk.com/woman/', 'https://sj.enterdesk.com/anime/',
                'https://sj.enterdesk.com/gaoxiaotupian/']
...
def get1list():
    ...
    for i in range(2, 6):
        for j in range(1, 20):
            ...
            obj = re.compile(
                '<li.*?cxudy-list-format"><a href="(?P<href>.*?)".*?data-original="(?P<src>.*?)".*?alt="(?P<name>.*?)"',
                re.S)
    ...
    for i in range(3, 6):
        with ThreadPoolExecutor(200) as t:
            for j in range(1, 1400):
                ...
                res = requests.get(url)
                obj = re.compile(
                    '<li><a class="thumbnail".*?src="(?P<src>.*?)".*?"(?P<name>.*?)".*?href="(?P<href>.*?)"', re.S)
                ls = obj.findall(res.text)
...
def get_search_list(keys, page):
    ...
    for it in its:
        ...
        name = it.group('name').replace('/', '-').replace('\\', '-')
        i += 1
...
def get_bili_src(href):
    res = requests.get(href)
    obj = re.compile(
        '<title data-vue-meta="true">(?P<name>.*?)</title>.*?"video":.*?"baseUrl":"(?P<video>.*?)".*?"audio":.*?"baseUrl":"(?P<audio>.*?)"',
        re.S)
...
def get_next(link, num):
    ...
    url = link
    res = requests.get(url)
    obj = re.compile('framepreview-box"><a href="(?P<href>.*?)".*?<p title="(?P<name>.*?)".*?<div class="upname">',
                     re.S)
...
def get_vi(request):
    ...
    if link:
        get_nextvi(link, num)
    if keys and pages:
        audio_downloadvi(keys, int(pages))
    res = Vi.objects.filter().all()
    for i in range(len(res)):
        res[i].visrc =  str(res[i].visrc, 'utf-8')
        res[i].ausrc =  str(res[i].ausrc, 'utf-8')
        res[i].name = str(res[i].name, 'utf-8')
    return render(request, 'vilist.html', {"results": res, "keys": keys, "pages": pages, "link": link, "num": num})
...
def see_vi(request):
    if request.session['info']['id'] != "1":
        return HttpResponse({"msg": "你没有权限"})
    id = request.GET.get('id')
    au = Vi.objects.filter(id=id).first()
    name = str(au.name, 'utf-8')
    ausrc = str(au.ausrc, 'utf-8')
    visrc = str(au.visrc, 'utf-8')
    return render(request, 'seevi.html', {"name": name, "ausrc": ausrc,"visrc":visrc})



def delete_vi(request):
    id = request.GET.get("uid")
    # delete_file('')
    visrc=Vi.objects.filter(id=int(id)).first().visrc
    ausrc=Vi.objects.filter(id=int(id)).first().ausrc
    delete_file(visrc)
    delete_file(ausrc)
    Vi.objects.filter(id=int(id)).delete()
    notify = []
    notify.append(dict(id=0, tittle="提示", context="音频 {} 删除成功".format(id), type="success", position="top-center"))
    request.session["notify"] = notify
...
def get_search_vilist(keys, page):
    url = f"https://search.bilibili.com/all?keyword={keys}&from_source=webtop_search&spm_id_from=333.1007&search_source=2&page={page}&o=360"
    res = requests.get(url)
    obj = re.compile('<li class="video-item matrix"><a href="//(?P<href>.*?)".*?title="(?P<name>.*?)"', re.S)
    its = obj.finditer(res.text)
    ...
    for it in its:
        ...
        name = it.group('name').replace('/', '-').replace('\\', '-')
        i += 1
        ausrc,visrc,s = get_bili_visrc(href)
        addrau = "/static/au/" + name + ".mp3"
        addrvi = "/static/vi/" + name + ".mp4"
        if not Vi.objects.filter(name=name).filter().first():
            save_bili_visrc(href, ausrc,visrc, name)
            Vi.objects.create(name=name,ausrc=addrau,visrc=addrvi)
        print(i)


def get_bili_visrc(href):
    res = requests.get(href)
    obj = re.compile(
        '<title data-vue-meta="true">(?P<name>.*?)</title>.*?"video":.*?"baseUrl":"(?P<video>.*?)".*?"audio":.*?"baseUrl":"(?P<audio>.*?)"',
        re.S)
...
def audio_downloadvi(keys, pages):
    with ThreadPoolExecutor(20) as t:
        for i in range(1, pages):
            t.submit(get_search_vilist, keys, i)
            print(i)


def get_nextvi(link, num):
    # url='https://www.bilibili.com/video/BV14k4y167Qc/'
    url = link
    res = requests.get(url)
    obj = re.compile('framepreview-box"><a href="(?P<href>.*?)".*?<p title="(?P<name>.*?)".*?<div class="upname">',
                     re.S)
    ausrc,visrc,name = get_bili_visrc(url)
    name = "".join(name.split(' ')).strip('_哔哩哔哩_bilibili').split("|")[0]
    addrau = "/static/au/" + name + ".mp3"
    addrvi = "/static/vi/" + name + ".mp4"
    if not Vi.objects.filter(name=name).filter().first():
        Vi.objects.create(ausrc=addrau,visrc=addrvi, name=name)
        save_bili_visrc(url, ausrc,visrc, name)
        print('over')
    n = 0
    for i in obj.finditer(res.text):
        n += 1
