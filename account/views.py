...
def ac_detail(request):
    ...
    ac = models.Yuangong.objects.filter(id=id).values("office", 'password', 'username', "email", "isactive",
                                                      "businessid_id").first()
    if not ac:
...
def ac_delete(request):
    ...
    if gc or gu or cc or cu or xj or cg:
        notify.append(
            dict(id=0, tittle="提示", context="员工 {} 有关联的供应商或待处理订单，不能删除".format(id), type="error", position="top-center"))
        request.session["notify"] = notify
